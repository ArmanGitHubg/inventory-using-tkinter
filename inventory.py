...
def Database():
    ...
    cursor.execute("Create Table if not exists 'admin' (admin_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, product_name TEXT, product_qty TEXT, product_price TEXT)")
...
def Exit():
    result =tkMessageBOX.askquestion('Python Inventroy Management System','Are you want to exit ?', icon ="warning")
    if result == 'yes':
        root.destroy()
        exit()
...
def AddNewForm():
    ...
    lbl_product_name = Label(MidAddNew, text="Product Name:", font=('arial',26),bd=10)
    lbl_product_name.grid(row=0,sticky=W)
...
def ViewForm():
    ...
    tree = ttk.Treeview(MidViewForm, columns=("ProductID","Product Name","Product Qty","Product Price"),
                         selectmode="extended", height=100, yscollcommand=scrollbary.set,xscrollcomand=scrollbarx.set)
...
def Delete():
    if not tree.selection():
        ...
        result = tkMessageBOX.askquestion('Python Inventory Management System', 'Are you wnat to delete with Recod?',icon="warning")
        if result =='yes':
            curItem = tree.focus()
            ...
            tree.delete(curItem)
            Database()
            cursor.execure("DELETE FROM 'product' WHERE 'product_id' =%d" % selecteditem[0])
            conn.commit()
            cursor.close()
            conn.close()
...
def Logout():
    result = tkMessageBOX.askquestion('Python Inventory Management System','Are you want to Logout? ',icon="warning")
    if result =='yes':
        admin_id = ""
        root.deiconify()
        Home.destroy()
...
lbl_diplay = Label(Title,text="Python Inventory Management System", font=('arial',35))
lbl_diplay.pack()
